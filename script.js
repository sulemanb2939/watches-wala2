...
function renderProducts() {
    ...
    grid.innerHTML = products.map(product => {
        ...
        return `
        <div class="product-card ${!product.stock ? 'out-of-stock-card' : ''}" onclick="window.location.href='product.html?id=${product.id}'" style="cursor: pointer;">
    ...
    list.innerHTML = cart.map((item, index) => {
        ...
        return `
            <li>
                ...
                <i class="fa-solid fa-trash" style="color:red; cursor:pointer; margin-left:10px;" onclick="window.removeFromCart(${index})"></i></span>
            </li>
        `;
    }).join('');
    updateFinalPrice();
